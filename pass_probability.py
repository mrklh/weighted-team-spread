...
class PitchValue:
    ...
    def get_defender_positions(self):
        def_players = [x[NAME] for x in filter(lambda x: x[0] == 101, pv.data[0])]
        self.def_players_data_at_0 = [[filter(lambda x: x[NAME] == z, y)[0] for y in pv.data[:2]] for z in def_players]
    ...
    def add_defensive_players(self):
        ...
        for player in self.def_players_data_at_0:
            sec1 = player[0]
            sec2 = player[1]
    ...
    def show(self):
        ...
        plt.colorbar(im)
        self.scat_players_and_ball()
        # plt.savefig('defensive_influence.pdf')
        plt.show()

    def scat_players_and_ball(self):
        for player in self.def_players_data_at_0:
            x, y = self.get_xy(player[0])
            plt.scatter(x, y, c='red', s=100)

        plt.scatter(self.ball_pos_dict[self.data[0][0][SEC]][0],
                    self.ball_pos_dict[self.data[0][0][SEC]][1],
                    c='white', s=100)

    def show_all(self):
        self.get_defender_positions()
